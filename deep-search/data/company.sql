...
create database if not exists pharma character set utf8mb4 collate utf8mb4_general_ci;
grant all privileges on pharma.* to 'study'@'%' with grant option;


use pharma;

-- 1. 药品信息表：记载每一种药品的详情信息
...
(
    ...
    approval_number  varchar(50)                             comment '批准文号 (国药准字H...)',
    specifications   varchar(100)                            comment '规格 (如 0.3g*24粒/盒)',
    dosage_form      varchar(50)                             comment '剂型 (胶囊/片剂/注射液)',
    manufacturer     varchar(100)                            comment '生产厂家',
    therapeutic_area varchar(50)                             comment '治疗领域 (如 解热镇痛, 心血管)',
    description      text                                    comment '药品详情/适应症描述',
    created_at       timestamp    default current_timestamp  comment '创建时间'
) char set utf8mb4 engine=innodb comment='药品信息表';

-- 2. 库存表：记录药品的库存量，通过 drug_id 关联
drop table if exists inventory;
create table if not exists inventory
(
    inventory_id       int          primary key   auto_increment                          comment '库存ID',
    drug_id            int          not null                                              comment '药品ID',
    batch_number       varchar(50)  not null                                              comment '生产批号 (医药库存核心字段)',
    quantity_on_hand   int          default 0                                             comment '当前库存量 (盒/瓶)',
    warehouse_location varchar(50)                                                        comment '仓库位置 (如 A区-01架)',
    production_date    date                                                               comment '生产日期',
    expiry_date        date                                                               comment '有效期至 (用于预警)',
    last_updated       timestamp    default current_timestamp on update current_timestamp comment '最后更新时间',
    foreign key (drug_id) references drugs (drug_id) on delete cascade
) char set utf8mb4 engine=innodb comment='库存表';

-- 3. 药品销售情况表：记录每个药品的销售情况，通过 drug_id 关联
...
(
    ...
    total_amount  decimal(15, 2)                              comment '销售总额',
    customer_name varchar(100)                                comment '客户名称 (如 xx第一人民医院, xx大药房)',
    region        varchar(50)                                 comment '销售区域 (用于区域分析)',
    sales_rep     varchar(50)                                 comment '销售代表',
    foreign key (drug_id) references drugs (drug_id) on delete cascade
) char set utf8mb4 engine=innodb comment='药品销售情况表';


-- 1. 插入 10 种药品信息 (全中文，假设均为我司生产)
insert ignore into drugs (generic_name, brand_name, approval_number, specifications, dosage_form, manufacturer, therapeutic_area, description)
values ('阿莫西林胶囊', '阿莫仙', '国药准字H20051234', '0.25g*24粒', '胶囊剂', '本公司制药', '抗生素',
        '用于治疗敏感菌引起的上呼吸道感染、泌尿生殖道感染等。'),
       ('布洛芬缓释胶囊', '芬必得', '国药准字H10900089', '0.3g*20粒', '胶囊剂', '本公司制药', '解热镇痛',
        '用于缓解轻至中度疼痛如头痛、关节痛、偏头痛、牙痛、肌肉痛、神经痛、痛经。也用于普通感冒或流行性感冒引起的发热。'),
       ('盐酸二甲双胍片', '格华止', '国药准字H20023345', '0.5g*48片', '片剂', '本公司制药', '糖尿病',
        '首选用于单纯饮食控制不满意的2型糖尿病患者，尤其是肥胖者。'),
       ('阿托伐他汀钙片', '立普妥', '国药准字H20055567', '20mg*7片', '片剂', '本公司制药', '心血管',
        '原发性高胆固醇血症患者，包括家族性高胆固醇血症（杂合子型）或混合性高脂血症。'),
       ('磷酸奥司他韦胶囊', '达菲', '国药准字H20090123', '75mg*10粒', '胶囊剂', '本公司制药', '抗病毒',
        '用于成人和1岁及1岁以上儿童的甲型和乙型流感治疗。'),
       ('注射用头孢曲松钠', '罗氏芬', '国药准字H10920012', '1.0g/支', '注射剂', '本公司制药', '抗生素',
        '用于敏感致病菌所致的下呼吸道感染、尿路、胆道感染，以及腹腔感染、盆腔感染、皮肤软组织感染、骨和关节感染、败血症、脑膜炎等。'),
       ('蒙脱石散', '思密达', '国药准字H20000456', '3g*10袋', '散剂', '本公司制药', '消化系统',
        '用于成人及儿童急、慢性腹泻。'),
       ('硝苯地平控释片', '拜新同', '国药准字H20100345', '30mg*30片', '片剂', '本公司制药', '高血压',
        '1.高血压。2.冠心病。慢性稳定型心绞痛（劳累性心绞痛）。'),
       ('阿司匹林肠溶片', '拜阿司匹灵', '国药准字J20130078', '100mg*30片', '片剂', '本公司制药', '心血管',
        '降低急性心肌梗死疑似患者的发病风险；预防心肌梗死复发。'),
       ('连花清瘟胶囊', '连花清瘟', '国药准字Z20040063', '0.35g*24粒', '胶囊剂', '本公司制药', '中成药/感冒',
...
insert ignore into inventory (drug_id, batch_number, quantity_on_hand, warehouse_location, production_date, expiry_date)
values
    -- 1. 阿莫西林
...
insert ignore into sales_records (drug_id, sale_date, quantity_sold, unit_price, total_amount, customer_name, region, sales_rep)
values
    -- 1. 阿莫西林
